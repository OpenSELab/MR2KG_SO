...
# 读取json文件
def read_write_json(path, mode, load_dict=None):
    if mode == 'r':
        try:
            ...
            with open(path, mode='w', encoding='utf-8') as fw:
                json.dump({}, fw, ensure_ascii=False)
            return {}
    if mode == 'w':
        with open(path, mode='w', encoding='utf-8') as fw:
            json.dump(load_dict, fw, ensure_ascii=False)


# 通过API获取SO数据，query为查询关键词，返回结果为相关的问题链接，详细数据保存在json文件中
def search_by_query(query):
    print(f"====searching {query} ...")
    query_string = {'q': query}
    encoding_query = parse.urlencode(query_string)
    page_num = 0
    has_more = True
    query_result = []
    while has_more and page_num < 25:
        page_num += 1
        url = f'https://api.stackexchange.com/2.3/search/advanced?page={page_num}&pagesize=100&order=desc&sort=relevance&{encoding_query}&site=stackoverflow'
        response = requests.get(url)
        response_json = json.loads(response.text)
        if 'has_more' in response_json.keys():
            has_more = response_json['has_more']
        else:
            has_more = False
        query_result.extend(response_json['items'])
        for item in response_json['items']:
            print(item['link'])
        break
    read_write_json(f'{encoding_query}.json', 'w', query_result)
    return query_result[0]["question_id"]
...
query = input("请输入需要搜索的关键字：")
id = search_by_query(query)
