...
from transformers import (AdamW, get_linear_schedule_with_warmup,
                          ...
                          RobertaTokenizer, AutoModel, BertTokenizer, BertConfig)
...
def evaluate(args, model, tokenizer):
    ...
    preds_proba = F.softmax(logits_tensor, dim=1)
    ...
    eval_auc = roc_auc_score(labels, preds_proba, multi_class='ovo')
    ...
    result = {
        ...
        "eval_mcc": round(float(eval_mcc), 4),
...
def main():
    ...
    # # 配置CodeBERT
    # config4Code = RobertaConfig.from_pretrained(args.model_name_or_path)
    # config4Code.num_labels = args.num_class
    # # tokenizer4Code = RobertaTokenizer.from_pretrained(args.tokenizer_name)
    # codeEncoder = AutoModel.from_pretrained(args.model_name_or_path, config=config4Code)
    ...
    textEncoder = AutoModel.from_pretrained(args.model_name_or_path, config=config)
    textEncoder1 = AutoModel.from_pretrained(args.model_name_or_path, config=config)

    model = BTModel(textEncoder, textEncoder1,
                    config.hidden_size, config.hidden_size, args.num_class)
